...
|---|---|---|
| **ChatGPT** | Deep Research | ResearchIt first runs an autonomous clarification + prompt-rewrite prep pass, then calls the OpenAI Responses API with model `o3-deep-research`, background mode enabled (`background: true`, `store: true`), `web_search_preview`, and `code_interpreter` (`container: auto`). The response is polled to completion. |
