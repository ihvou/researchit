  ...
  return {
    async callAnalyst(messages, systemPrompt, maxTokens = 5000, options = {}) {
      const payload = {
        ...
        searchMaxUses: Number.isFinite(Number(options.searchMaxUses)) ? Math.max(1, Math.floor(Number(options.searchMaxUses))) : undefined,
        ...
        chunkId: typeof options.chunkId === "string" ? options.chunkId : undefined,
        callIndex: Number.isFinite(Number(options.callIndex)) ? Number(options.callIndex) : undefined,
        promptVersion: typeof options.promptVersion === "string" ? options.promptVersion : undefined,
