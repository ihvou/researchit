...
function suggestedQueries({ dim, title, vertical, dimLabel, missingEvidence, aiType }) {
  ...
  const templates = normalizeList(dim?.researchHints?.queryTemplates, 6);
  if (templates.length) {
    const rendered = templates.map((tpl) => renderTemplate(tpl, {
      title: baseTitle || "use case",
      vertical: baseVertical || "enterprise",
      dimLabel: baseDim || "dimension",
      missingEvidence: gap || "evidence gap",
      aiType: ai || "AI",
    }));
    return normalizeList(rendered, 4);
  }
...
export function getResearchBriefForLowConfidence({
  ...
  return {
    ...
    whereToLook: whereToLookByDimension(dim, dimId, attributes?.vertical, attributes?.aiSolutionType),
