...
async function verifyStrict(source = {}, fetchSource, cache, options = {}) {
  const url = clean(source?.url);
  const payload = await fetchWithCache(url, fetchSource, cache, { resolveOnly: false, timeoutMs: options?.timeoutMs || 12000 });
...
async function verifyExistence(source = {}, fetchSource, cache, options = {}) {
  const url = clean(source?.url);
  const payload = await fetchWithCache(url, fetchSource, cache, { resolveOnly: true, timeoutMs: options?.resolveTimeoutMs || 9000 });
  const status = statusCodeFromPayload(payload);
  const fetchStatus = clean(payload?.sourceFetchStatus);
...
async function verifyAnalyst(source = {}, fetchSource, cache, options = {}) {
  const url = clean(source?.url);
  const probe = await fetchWithCache(url, fetchSource, cache, { resolveOnly: true, timeoutMs: options?.resolveTimeoutMs || 9000 });
  ...
  const strict = await verifyStrict(source, fetchSource, cache, options);
...
export function createVerificationCounters() {
  return {
    checked: 0,
    verified: 0,
    notFound: 0,
...
export async function verifySourcesForUnit(
  unit = {},
  {
    fetchSource,
    cache = new Map(),
    timeoutMs = 12000,
    resolveTimeoutMs = 9000,
  } = {}
) {
  const counters = createVerificationCounters();
  const sources = ensureArray(unit?.sources);
  if (typeof fetchSource !== "function") return counters;
  ...
  for (const source of sources) {
    ...
    if (isGroundingRedirectUrl(url)) {
      const probe = await fetchWithCache(url, fetchSource, cache, { resolveOnly: true, timeoutMs: resolveTimeoutMs });
    ...
    const options = { timeoutMs, resolveTimeoutMs };
    const verdict = tier === "existence"
      ? await verifyExistence(source, fetchSource, cache, options)
      : (tier === "analyst"
        ? await verifyAnalyst(source, fetchSource, cache, options)
        : await verifyStrict(source, fetchSource, cache, options));
...
export function aggregateVerificationCounters(counters = []) {
  return ensureArray(counters).reduce((acc, item) => {
