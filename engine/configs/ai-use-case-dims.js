...
const DEFAULT_RESEARCH_HINTS = {
  ...
  evidence: {
    whereToLook: [
      "Peer-reviewed or audited deployments with measurable outcomes.",
      "Industry publications citing named implementations and timelines.",
      "Operator case studies with pre/post metrics.",
    ],
    queryTemplates: [
      "${title} ${vertical} named deployment case study",
      "${vertical} benchmark ${dimLabel} pre post metrics",
  ...
  build_vs_buy: {
    whereToLook: [
      ...
      "Third-party competitive analyses for current vendor coverage.",
      "RFPs / practitioner writeups on customization requirements.",
    ],
    queryTemplates: [
...
const FALLBACK_RESEARCH_HINTS = {
  whereToLook: [
    "Independent analyst reports and benchmark studies.",
    "Named deployment case studies from operators, not only vendor blogs.",
    "Internal delivery post-mortems and client references.",
  ],
  queryTemplates: [
    "${title} ${vertical} ${dimLabel} deployment metrics",
    "${vertical} ${aiType} case study audited outcomes ${dimLabel}",
    "${missingEvidence} ${vertical} source",
  ],
};

export const DEFAULT_DIMS = BASE_DIMS.map((dim) => ({
  ...dim,
  polarityHint: dim.polarityHint || DEFAULT_POLARITY_HINTS[dim.id] || "Higher score = stronger evidence-backed attractiveness for this decision.",
  researchHints: {
    whereToLook: dim.researchHints?.whereToLook || DEFAULT_RESEARCH_HINTS[dim.id]?.whereToLook || FALLBACK_RESEARCH_HINTS.whereToLook,
    queryTemplates: dim.researchHints?.queryTemplates || DEFAULT_RESEARCH_HINTS[dim.id]?.queryTemplates || FALLBACK_RESEARCH_HINTS.queryTemplates,
  },
}));
